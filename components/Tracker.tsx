 ...
 * Renders GPS coordinates relative to the starting point.
 */
const KinematicCanvas: React.FC<{ path: Coordinate[], isTracking: boolean, isBlocked: boolean }> = ({ path, isTracking, isBlocked }) => {
  ...
  useEffect(() => {
    ...
    // Draw Vector Path
    ctx.beginPath();
    ctx.strokeStyle = isBlocked ? '#22d3ee' : 'rgba(34, 211, 238, 0.6)';
    ctx.lineWidth = 4;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ctx.shadowBlur = 10;
...
const Tracker: React.FC<TrackerProps> = ({ onSaveRun, userWeightLbs }) => {
  ...
  useEffect(() => {
    // Global handler for Google Maps Auth Failures (removes the "Oops" div)
    (window as any).gm_authFailure = () => {
      console.warn("Terminal: Google Maps Authentication Failure Detected.");
      setMapBlocked(true);
    };
    ...
    const initialize = async () => {
      try {
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          () => {
            console.warn("Terminal: Could not acquire initial GPS position.");
  ...
  const startTracking = () => {
    ...
    watchId.current = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        // Filter: Ignore updates with poor accuracy (>50m) to reduce stationary jitter
        ...
        if (lastCoordRef.current) {
          ...
          // Denoise: Only accumulate distance if jump is significant (> 10 meters)
          // or speed is verified as > 0.5 m/s (approx 1.1 mph)
          const isMoving = distMi > 0.006 || (speed !== null && speed > 0.5);
          ...
          if (isMoving) {
            ...
            if (polylineRef.current && !mapBlocked) {
              try { polylineRef.current.getPath().push(new google.maps.LatLng(lat, lng)); } catch(e) {}
            }
      ...
      (err) => {
        console.warn("Terminal: GPS Signal Interrupted.", err.message);
        setHasGpsFix(false);
      },
  ...
  // Pace display logic: Don't show wild numbers if distance is negligible
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            <p className="text-[10px] font-mono text-slate-500 uppercase tracking-widest flex items-center gap-2">
              ...
              SYSTEM_LINK: {isTracking ? "ACTIVE" : "STANDBY"}
            </p>
            {mapBlocked && (
              <span className="flex items-center gap-1 text-[9px] font-black text-cyan-400 bg-cyan-400/10 px-2 py-0.5 rounded border border-cyan-400/20 uppercase tracking-tighter">
                VECTOR_SIMULATION_V2
              </span>
            )}
          </div>
        </div>
        <div className="flex gap-2">
          <button 
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl border transition-all ${isVoiceEnabled ? 'bg-cyan-500/20 border-cyan-500/40 text-cyan-400' : 'bg-slate-800 border-slate-700 text-slate-500'}`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <div className="h-[480px] w-full bg-slate-950 border border-slate-800 rounded-[2rem] sm:rounded-[3.5rem] relative overflow-hidden shadow-2xl">
            {/* The Google Maps layer is completely unmounted or hidden if blocked to remove the "Oops" div */}
            {!mapBlocked ? (
            ...
            <div className="absolute top-6 left-6 sm:top-10 sm:left-10 z-20">
              <div className="bg-slate-950/80 backdrop-blur-2xl border border-white/5 px-4 py-2 sm:px-6 sm:py-3 rounded-2xl flex items-center gap-3 sm:gap-4 shadow-2xl">
                <div className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full animate-pulse ${mapBlocked ? 'bg-amber-500 shadow-[0_0_10px_orange]' : 'bg-cyan-500 shadow-[0_0_12px_cyan]'}`} />
            ...
            {isTracking && !hasGpsFix && (
               <div className="absolute bottom-6 left-6 z-20 flex items-center gap-2 bg-slate-950/80 border border-amber-500/20 px-3 py-1.5 rounded-xl text-[9px] font-black text-amber-500 uppercase tracking-widest animate-pulse backdrop-blur-md">
                  <Satellite size={12} /> AWAITING_KINETIC_SYNC...
               </div>
            )}
            ...
            {mapBlocked && !showErrorHelp && (
              <button 
                ...
                className="absolute top-6 right-6 z-20 bg-slate-900/90 border border-amber-500/20 px-3 py-1.5 rounded-xl flex items-center gap-2 text-[9px] font-black text-amber-500 uppercase tracking-widest hover:bg-slate-800 transition-colors"
              >
                <Info size={12} /> DEBUG_LOG
              </button>
            )}
            ...
            {showErrorHelp && (
              <div className="absolute inset-0 z-50 bg-slate-950/95 backdrop-blur-xl flex items-center justify-center p-8">
                <div className="max-w-xs text-center space-y-4">
                  ...
                  <h4 className="text-white font-black uppercase text-xs tracking-widest">MAP_LOCK_RESTRICTED</h4>
                  <p className="text-slate-400 text-[10px] font-mono leading-relaxed">
                    Host domain is not authorized for the Satellite imagery layer. Vector Kinematics remain 100% operational.
                  </p>
                  <button 
...
const TelemetryCard = ({ icon, label, value, color = "text-white", isActive = false, highlight = false }: { icon: any, label: string, value: string, color?: string, isActive?: boolean, highlight?: boolean }) => (
  <div className={`bg-slate-900/60 border border-slate-800 p-6 sm:p-10 rounded-[2rem] sm:rounded-[3rem] shadow-xl backdrop-blur-md transition-all hover:bg-slate-900/80 ${highlight && isActive ? 'ring-2 ring-cyan-500/30 border-cyan-500/20 shadow-[0_0_30px_rgba(34,211,238,0.05)]' : ''}`}>
    ...
    <div className={`text-2xl sm:text-4xl font-mono font-black tracking-tighter ${color}`}>
      {value}
    </div>
  </div>
);
